/* projects-mobile-preview.js
 * 모바일에서 스크롤을 멈추면 화면에 보이는 영상·상세페이지 미리보기를 실행합니다.
...
(() => {
  ...
  const entries = [...grid.querySelectorAll(".folder-card")]
    .map((card) => ({
      card,
      stage: card.querySelector(".folder-card-preview"),
  ...
  function start(entry) {
    ...
    function step(now) {
      ...
      if (distance > 0) {
        ...
        else if (time > pause * 2 + travel)
          progress = 1 - (time - pause * 2 - travel) / travel;
  ...
  // 화면에 충분히 보이는 미리보기 중 최대 2개를 골라 재생합니다.
  function selectVisible() {
    if (
      ...
      !grid.closest(".mobile-explorer-folder-open")
    ) {
      stopAll();
      return;
    }
    ...
    const headerBottom =
      document.querySelector(".browser-bar")?.getBoundingClientRect().bottom || 0;
    const candidates = entries
      .map((entry) => {
        const rect = entry.stage.getBoundingClientRect();
        const visible = Math.max(
          0,
          Math.min(rect.bottom, height) - Math.max(rect.top, headerBottom),
        );
        return {
          entry,
          rect,
          visible,
          distance: Math.abs((rect.top + rect.bottom) / 2 - height / 2),
        };
      })
      .filter(
        (item) =>
          item.rect.width > 0 &&
          item.rect.height > 0 &&
          item.visible / item.rect.height >= 0.7,
      )
