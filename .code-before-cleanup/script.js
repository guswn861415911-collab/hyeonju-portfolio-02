...
function setupCustomCursor() {
  ...
  function start() {
    ...
    cursor.style.display = ""; canvas.style.display = "";
    resizeCanvas();
  ...
  function animateTrail() {
    ...
    while (
      points.length > 0 &&
      points[points.length - 1].timestamp < cutoff
    ) {
      points.pop();
    }
    ...
    for (let index = 0; index < points.length; index += 1) {
      ...
      const ageFade = Math.max(
        0,
        1 - (now - point.timestamp) / TRAIL_LIFETIME
      );
  ...
  function stop() {
    ...
    frame = null; points.length = 0; hasMoved = false;
    previousX = pointerX; previousY = pointerY;
    context.clearRect(0, 0, canvas.width, canvas.height);
    document.body.classList.remove("has-custom-cursor");
    cursor.classList.remove("is-hovering");
    cursor.style.display = "none"; canvas.style.display = "none";
  }
  const control = document.createElement("div");
  control.id = "cursorEffectControl";
  control.innerHTML = '<div class="cursor-effect-label"><span aria-hidden="true">✨</span> Cursor Effect</div><button type="button" role="switch" aria-label="Cursor Effect"><span>OFF</span><span class="cursor-effect-track"><span></span></span><span>ON</span></button>';
  ...
  dragHandle.setAttribute("aria-label", "마우스 설정 창 이동. 방향키 또는 드래그로 이동할 수 있습니다.");
  ...
  function positionControl(left, top) {
    ...
    Object.assign(control.style, { left: x + "px", top: y + "px", right: "auto", bottom: "auto" });
  }
  function saveControlPosition() {
    const bounds = control.getBoundingClientRect();
    try { localStorage.setItem("cursorEffectPosition", JSON.stringify({ x: bounds.left, y: bounds.top })); } catch (_) {}
  }
  try {
    const saved = JSON.parse(localStorage.getItem("cursorEffectPosition"));
    if (saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)) positionControl(saved.x, saved.y);
  } catch (_) {}
  dragHandle.addEventListener("pointerdown", function (event) {
    if (!event.isPrimary || event.button !== 0) return;
    const bounds = control.getBoundingClientRect();
    controlDrag = { id: event.pointerId, x: event.clientX - bounds.left, y: event.clientY - bounds.top };
  ...
  for (const event of ["pointerup", "pointercancel", "lostpointercapture"]) dragHandle.addEventListener(event, finishControlDrag);
  ...
  let enabled = true;
  try { enabled = localStorage.getItem("cursorEffect") !== "off"; } catch (_) {}
  function apply() {
  ...
  toggle.addEventListener("click", function () {
    enabled = !enabled;
    try { localStorage.setItem("cursorEffect", enabled ? "on" : "off"); } catch (_) {}
    apply();
  });
  apply();
}
...
function setupProjectFilters() {
  ...
  function openMobileExplorer(filter, labelText) {
    ...
    const currentPath = explorerHeader.querySelector(
      ".mobile-explorer-current-path"
    );
  ...
  mobileProjects.addEventListener("change", function () {
    ...
    if (!mobileProjects.matches) {
      ...
      filterButtons.forEach(button => button.classList.toggle("selected", button.dataset.filter === "all"));
    }
  });
}
  ...
  document.querySelectorAll(".folder-card").forEach(function (card) {
    ...
    card.addEventListener("mouseenter", function () {
      ...
      videos.forEach(function (video) {
        ...
        if (playing) playing.then(function () {
          if (!hovering) { video.pause(); if (video.readyState >= 1) video.currentTime = 0; }
        }).catch(function () {});
      });
    });
...
function setupProjectVideoModal() {
  ...
  function openModal(button) {
    ...
    ["hunminQuote", "hunminNote", "hunminYoutubeLink", "hunminDescriptionHeading"].forEach(function(id) { const el = document.getElementById(id); if (el) el.hidden = !isStory; });
    ...
    if (titleEl) titleEl.textContent = story ? story.title : isHunmin ? "훈민정음 창제 :\n세종의 비밀 프로젝트, 글자 전쟁" : title;
    ...
    if (quoteEl) quoteEl.textContent = story ? story.quote : "“말할 수 있는 사람에게,\n자신의 뜻을 기록할 가능성을 건넨 설계였다.”";
    if (noteEl) noteEl.textContent = story ? "AI 이미지 생성, AI 영상 제작, AI 음악, AI 나레이션을 활용하여\n완성한 팀 프로젝트입니다." : "한글의 탄생을 현대적인 영상 언어로 재해석한 프로젝트.";
    if (youtubeLink) youtubeLink.href = "https://www.youtube.com/watch?v=" + (isHunmin ? "uudhaBaXUUs" : youtubeId);
    ...
    panel.querySelectorAll("[data-story-chip]").forEach(function(chip) { chip.remove(); });
    if (story) {
    ...
    if (orientation === "portrait") {
      ...
      if (isStory && hunminIframe) {
        ...
        hunminIframe.src = "https://www.youtube-nocookie.com/embed/" + (isHunmin ? "uudhaBaXUUs" : youtubeId) + "?autoplay=1&mute=1&controls=1&playsinline=1&rel=0&start=0";
...
function fitProjectOverview(element) {
  if (window.matchMedia('(max-width: 767px)').matches) {
    ['zoom', 'height', 'width', 'transform', 'transform-origin', 'position', 'inset', 'margin'].forEach(property => element.style.removeProperty(property));
    return;
  }
  ...
  const contentHeight = content ? content.offsetHeight : Math.max(
    element.querySelector(".detail-page-modal-copy")?.offsetHeight || 0,
    element.querySelector(".project-laptop-wrap")?.offsetHeight || 0);
  const height = Math.ceil(Math.max(element.offsetHeight,
    contentHeight + parseFloat(styles.paddingTop) + parseFloat(styles.paddingBottom)
      + parseFloat(styles.borderTopWidth) + parseFloat(styles.borderBottomWidth)));
  element.style.height = height + "px";
  const scale = Math.min(1, availableWidth / element.offsetWidth, availableHeight / height);
...
function setupDetailPageModal() {
  ...
  if (laptopScreen) laptopScreen.addEventListener("wheel", dismissScrollGuide, { passive: true });
  if (scrollArea) {
  ...
  modal.addEventListener("keydown", (event) => {
    if (["ArrowDown", "ArrowUp", "PageDown", "PageUp", "Home", "End", " "].includes(event.key)) dismissScrollGuide();
  });
  function closeModal() {
    stopScrollGuide();
    modal.hidden = true;
  ...
  "05": {
    ...
    "quote": "실사용 환경을 고려한 정보 설계와 시각적 구성으로, 구매자가 제품의 가치를 쉽게 이해할 수 있도록 디자인했습니다.",
  ...
  "06": {
    ...
    "quote": "제품의 컬러감과 실제 적용 이미지를 중심으로, 소비자가 구매 전 제품의 변화를 직관적으로 이해할 수 있도록 디자인했습니다.",
  ...
  "07": {
    ...
    "quote": "복잡한 제품 정보를 이해하기 쉬운 시각적 구성으로 정리하여, 제품의 성능과 장점을 효과적으로 전달하도록 디자인했습니다.",
  ...
  "08": {
    ...
    "quote": "제품의 핵심 정보를 직관적인 레이아웃과 시각적 흐름으로 구성하여, 구매자가 제품의 가치를 빠르게 이해할 수 있도록 디자인했습니다.",
    "note": "카즈온(주) 재직 기간 동안 제작한 실무 자동차 에어컨필터 상세페이지 디자인 프로젝트입니다."
  }
})[projectNum];
    panel.classList.add("is-marketing-detail");
    if (modalTitle) modalTitle.textContent = project?.title || d.projectTitle || "Detail Page";
    ...
    document.getElementById("detailProjectQuote").textContent = project ? "“" + project.quote + "”" : "";
...
document.addEventListener("DOMContentLoaded", function setupPlannerGallery() {
  document.querySelectorAll("#plannerModal, #diaryPlannerModal, #studyNotesModal, #lifePlannerModal").forEach(function(modal) {
  ...
  const cursorLayers = ["cursorTrailCanvas", "customCursor"].map(function(id) {
  ...
  function fitPlannerModal() {
    ...
    modal.style.setProperty('--planner-mobile-width', Math.round(Math.min(390, Math.max(220, window.innerHeight * .48 * ratio + 28))) + 'px');
  ...
  function syncPlannerPageRatio() {
    ...
    if (width > 0 && height > 0) {
      ...
      modal.style.setProperty("--planner-device-width", Math.min(640, Math.round(490 * width / height) + 32) + "px");
    }
    fitPlannerModal();
  }
  ...
  function showPage(index) {
    ...
    if (modal.open && oldSrc !== option.dataset.src && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
      const stage = image.parentElement;
      stage.querySelectorAll('[data-page-transition]').forEach(node => node.remove());
      const snapshot = image.cloneNode(false);
      snapshot.removeAttribute('id'); snapshot.removeAttribute('data-planner-image');
      snapshot.setAttribute('data-page-transition', ''); snapshot.alt = ''; snapshot.setAttribute('aria-hidden','true');
      snapshot.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:none;z-index:2';
      stage.append(snapshot);
      const direction = select.selectedIndex < oldIndex ? -1 : 1;
      const expected = option.dataset.src;
      image.addEventListener('load', function transitionPage() {
        if (image.getAttribute('src') !== expected || !snapshot.isConnected) { snapshot.remove(); return; }
        image.animate([{opacity:.3,translate:`${direction*18}px 0`},{opacity:1,translate:'0 0'}],{duration:320,easing:'ease-out'});
        snapshot.animate([{opacity:1,translate:'0 0'},{opacity:0,translate:`${-direction*18}px 0`}],{duration:320,easing:'ease-out'}).finished.finally(()=>snapshot.remove());
      }, {once:true});
      setTimeout(()=>snapshot.remove(),2000);
    }
    image.src = option.dataset.src;
    image.alt = modal.querySelector("h2").textContent + " · " + (option.dataset.colorLabel ? option.dataset.colorLabel + " · " : "") + label;
  ...
  if (colorButtons.length) {
    ...
    function chooseColor(colorIndex) {
      ...
      const pages = allPages.slice(colorIndex * pagesPerColor, (colorIndex + 1) * pagesPerColor);
      select.replaceChildren(...pages);
      pages.forEach(function(option, index) {
        option.textContent = String(index + 1).padStart(2, "0") + " · " + option.dataset.pageLabel;
      });
      colorButtons.forEach(function(button, index) {
        button.setAttribute("aria-pressed", String(index === colorIndex));
      });
      select.setAttribute("aria-label", colorButtons[colorIndex].textContent.trim() + " 페이지 선택");
      showPage(pageIndex);
    }
    ...
    colorButtons.forEach(function(button, index) {
      button.addEventListener("click", function() { chooseColor(index); });
    });
    chooseColor(0);
  }
  document.querySelectorAll('[data-planner-open][aria-controls="' + modal.id + '"]').forEach(function(button) {
  ...
  modal.querySelector(".planner-close").addEventListener("click", function() { modal.close(); });
  modal.addEventListener("click", function(event) {
  ...
  modal.addEventListener("close", function() {
    cursorLayers.forEach(function(layer) { layer.marker.after(layer.element); });
    document.body.classList.remove("modal-open");
    if (opener) opener.focus();
  });
  select.addEventListener("change", function() { showPage(select.selectedIndex); });
  modal.querySelector("[data-planner-prev]").addEventListener("click", function() { showPage(select.selectedIndex - 1); });
  modal.querySelector("[data-planner-next]").addEventListener("click", function() { showPage(select.selectedIndex + 1); });
