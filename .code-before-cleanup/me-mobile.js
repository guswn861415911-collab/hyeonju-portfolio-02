(() => {
  ...
  links.before(linksHome); keywords.before(keywordsHome);
  ...
  const groups = [
    ['Design','디자인 기초와 편집 작업의 시작.','◉',[['Photoshop','01_Adobe_Photoshop.png'],['Illustrator','02_Adobe_Illustrator.png'],['AutoCAD','03_AutoCAD.png']]],
    ['Web design','웹 퍼블리싱과 프론트엔드 구현.','‹/›',[['HTML',null,'5'],['CSS',null,'3'],['JavaScript',null,'JS']]],
    ['Video & Motion','영상 제작과 편집, 간단한 숏폼 제작.','▷',[['Premiere Pro','15_Premiere_Pro.png'],['After Effects','16_After_Effects.png'],['CapCut','04_CapCut.png']]],
    ['AI / Creative Tools','업무 효율을 높이는 AI 및 크리에이티브 도구.','✧',[['ChatGPT','07_ChatGPT.png'],['Gemini','06_Gemini.png'],['NotebookLM','11_NotebookLM.png'],['Gamma','12_Gamma.png'],['Canva','13_Canva.png'],['Figma','14_Figma.png'],['Notion',null,'N'],['기타 AI Tool',null,'+']]]
  ];
  ...
  function revealSkills() {
    ...
    skillFrame = requestAnimationFrame(() => {
      const play = (node, delay) => {
        if (!node) return;
        skillAnimations.push(node.animate(
          [{opacity:0, translate:'0 -14px'}, {opacity:1, translate:'0 0'}],
          {duration:520, delay, easing:'cubic-bezier(.22,1,.36,1)', fill:'backwards'}
        ));
      };
      root.querySelectorAll('.me-mobile-heading > *').forEach((node,i) => play(node,i*80));
      if (selected === 1) {
        contactPanel.querySelectorAll('.me-welcome > div > *').forEach((node,i) => play(node,220+i*80));
      ...
      skillPanel.querySelectorAll('.me-skill-group').forEach((group,i) => {
        ...
        group.querySelectorAll('.me-programs li').forEach((node,j) => play(node,start+100+j*40));
      });
      play(keywords,950);
    });
  }
  function select(index, focus = false) {
    ...
    skillPanel.hidden = index !== 0; contactPanel.hidden = index !== 1;
    root.dataset.active = index ? 'contact' : 'skills';
    root.querySelector('h1').textContent = index ? 'Contact' : 'Skills';
    root.querySelector('.eyebrow').textContent = index ? 'ME · GET IN TOUCH' : 'ME · MY TOOLKIT';
    root.querySelector('.me-mobile-intro').textContent = index ? '협업 문의와 프로젝트 제안은 아래 연락처 및 링크를 통해 편하게 연락 주세요.' : '지금까지 익혀온 도구와 프로그램, 그리고 실제 작업에 활용하고 있는 스킬들을 정리했습니다.';
  ...
  root.querySelector('[role="tablist"]').addEventListener('keydown', event => {
    ...
    event.preventDefault(); select(event.key==='Home'?0:event.key==='End'?1:1-selected,true);
  });
  function arrange() {
    if (mobile.matches) { skillPanel.append(keywords); contactPanel.append(links); }
    else { keywordsHome.after(keywords); linksHome.after(links); }
  }
