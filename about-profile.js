/* about-profile.js
 * About의 6개 슬라이드를 만들고, 모바일과 PC에 맞게 요소를 이동합니다.
...
(() => {
  ...
  stats.innerHTML =
    '<p>ABOUT ME</p><div class="about-stat-pair"><div><strong>5+</strong><span>Years<br>Experience</span></div><div><strong>16</strong><span>Projects</span></div></div><div class="about-stat-design"><strong>Design × AI</strong><span>Visual Designer<br>Content Creator</span></div>';
  ...
  cards.forEach((card, i) => {
    ...
    card.setAttribute(
      "aria-label",
      `${i + 1} / ${cards.length}: ${card.querySelector("h3").textContent}`,
    );
  ...
  controls.innerHTML =
    '<button class="about-slide-prev" type="button" aria-label="이전 프로필 항목"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14 5-7 7 7 7"/></svg></button><div class="about-slide-dots"></div><button class="about-slide-next" type="button" aria-label="다음 프로필 항목"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m10 5 7 7-7 7"/></svg></button>';
  ...
  function show(index, announce = true) {
    ...
    if (announce && !matchMedia("(prefers-reduced-motion: reduce)").matches)
      cards[active].animate(
        [
          { opacity: 0, translate: `${direction * 22}px 0` },
  ...
  controls
    .querySelector(".about-slide-prev")
    .addEventListener("click", () => show(active - 1));
  controls
    .querySelector(".about-slide-next")
    .addEventListener("click", () => show(active + 1));
